...
def format_amount(val) -> str:
    """Return integer string if whole number, else keep decimals as-is."""
    if pd.isna(val):
        return ""
...
def generate_ach_csv(df: pd.DataFrame) -> tuple[str, list[int], list[int], str]:
    ...
    for i, rec in df.iterrows():
        ...
        batch_row = [
            ...
            "CCD", rec["Stock # Prefix"], payment_date,
            "", "", "", "", "", "", "",
        ]
        detail_row = [
            "R", "", "", "", "", "",
            rec["Account Name"],
            str(int(rec["Routing Number"])).zfill(9),   # preserve leading zeros
...
if uploaded_file:
    ...
    with st.expander("Preview source data"):
        st.dataframe(df[["Account Name", "Routing Number", "Account Number", "Amount", "Stock # Prefix"]])
